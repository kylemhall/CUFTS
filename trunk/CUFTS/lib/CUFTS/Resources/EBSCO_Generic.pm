...
sub resource_details_help {
	...
	$help_hash->{'resource_identifier'} = "This is a three character code that EBSCO uses to uniquely identify some databases.\n\nExample: AFH";
...
sub build_linkFulltext {
        ...
        defined($request) or
                CUFTS::Exception::App->throw('No request defined in build_Fulltext');
...
sub build_linkTOC {
	...
	my %search_fields = %{$class->_search_fields};                
		...
		$url .= $class->_build_journal_search_field($record->title, $record->issn);                
                ...
                $url .= "+AND+$search_fields{'volume'}+" . $request->volume;
		$url .= "+AND+$search_fields{'issue'}+" . $request->issue if $request->issue;
