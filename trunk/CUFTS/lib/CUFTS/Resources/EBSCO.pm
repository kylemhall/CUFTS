...
sub build_linkJournal {
	...
	foreach my $record (@$records) {
		...
		my $db = $resource->resource_identifier or next;
		my $journal_id = $record->details->db_identifier or next;
		$db = lc($db);

		my $result = new CUFTS::Result("http://search.epnet.com/direct.asp?db=${db}&jid=\%22${journal_id}\%22&scope=site");
...
sub build_linkDatabase {
	...
	foreach my $record (@$records) {
		...
		if (assert_ne($resource->database_url)) {
			$url .= $resource->database_url;
		} else {
			$url .= "http://search.epnet.com/login.asp?profile=web&defaultdb=$db";
		}
...
sub produce_ebsco_xml {
	...
	assert_ne($resource->resource_identifier) or
...
;
	...
	if (defined($request->issue) || defined($request->volume) || defined($request->date)) {
		$xml_string .= "   <pubinfo>\n";		
