...
sub clean_data {
	...
	defined($record->{'issn'}) and
		$record->{'issn'} =~ s/[^xX0-9]//g;
	defined($record->{'e_issn'}) and
		$record->{'e_issn'} =~ s/[^xX0-9]//g;
	
	my $start = $record->{'___EJS Coverage From'};
	my $end = $record->{'___EJS Coverage To'};

	defined($start) && $start =~ m#n/a# and
		$start = undef;
	defined($end) && $end =~ m#n/a# and
		$end = undef;
	
	if (defined($start)) {
		$start =~ s/\s*released\s*//i;
		$start =~ s/\s*quarter\s*//i;
		$start =~ s/\(No\.\s*/(/i;

		$start =~ /volume\s+-?([0-9]+)/i and
			$record->{'vol_ft_start'} = $1;

		if ($start =~ /numbers\s+([0-9]+)/i) {
			$record->{'iss_ft_start'} = $1;
		} elsif ($start =~ /number\s+-?([0-9]+)/i) {
			$record->{'iss_ft_start'} = $1;
		}

		if ($start =~ /\(\s*([A-Za-z]+)\s+(\d+)\s*,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_start_date'} = sprintf("%04i-%02i-%02i", $3, get_month($1, 'start'), $2);
		} elsif ($start =~ /\(\s*([A-Za-z]+)[^A-Za-z]+[A-Za-z]+,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_start_date'} = sprintf("%04i-%02i", $2, get_month($1, 'start'));
		} elsif ($start =~ /\(\s*([A-Za-z]+)\s*,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_start_date'} = sprintf("%04i-%02i", $2, get_month($1, 'start'));
		} elsif ($start =~ /\(\s*(\d{4})\s*\)/) {
	...
	if (defined($end)) {
		$end =~ s/\s*released\s*//i;
		$end =~ s/\s*quarter\s*//i;
		$end =~ s/\(No\.\s*/(/i;

		$end =~ /volume\s+-?([0-9]+)/i and
			$record->{'vol_ft_end'} = $1;

		if ($end =~ /numbers\s+[0-9]+\s*[-\/]\s*([0-9]+)/i) {
			$record->{'iss_ft_end'} = $1;
		} elsif ($end =~ /number\s*-?\s*([0-9]+)/i) {
			$record->{'iss_ft_end'} = $1;
		}

		if ($end =~ /\(\s*([A-Za-z]+)\s+(\d+)\s*,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_end_date'} = sprintf("%04i-%02i-%02i", $3, get_month($1, 'end'), $2);
		} elsif ($end =~ /\(\s*[A-Za-z]+\s*[^A-Za-z\s]\s*([A-Za-z]+)\s*,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_end_date'} = sprintf("%04i-%02i", $2, get_month($1, 'end'));
		} elsif ($end =~ /\(\s*([A-Za-z]+)\s*,?\s*(\d{4})\s*\)/i) {
			$record->{'ft_end_date'} = sprintf("%04i-%02i", $2, get_month($1, 'end'));
		} elsif ($end =~ /\(\s*(\d{4})\s*\)/) {
			$record->{'ft_end_date'} = $1;
		}

		if (defined($record->{'ft_end_date'}) && $record->{'ft_end_date'} =~ /^2004/) {
...
sub build_linkJournal {
	...
	foreach my $record (@$records) {
		...
		my $id = $record->db_identifier or next;
		
		my $result = new CUFTS::Result;
		$result->url("http://ejournals.ebsco.com/direct.asp?JournalID=$id");
...
sub build_linkDatabase {
	...
	foreach my $record (@$records) {
		my $result = new CUFTS::Result;
		$result->url('http://ejournals.ebsco.com');
		$result->record($record);
