...
sub clean_data {
	...
	if (defined($record->{'___Availability'}) && $record->{'___Availability'} =~ /(\d{4})\s*\-\s*(\d{4})/) {
		$record->{'ft_start_date'} = $1;
		$record->{'ft_end_date'} = $2;
	} elsif (defined($record->{'___Availability'}) && $record->{'___Availability'} =~ /(\d{4})/) {
		$record->{'ft_start_date'} = $1;
	}
...
sub skip_record {
	...
	return assert_ne($record->{'___ISBN'});
}
