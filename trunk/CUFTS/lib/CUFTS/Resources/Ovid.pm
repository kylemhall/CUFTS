...
sub clean_data {
    ...
    if ( not_empty($record->{ft_start_date}) && $record->{ft_start_date} !~ /^\d{4}-\d{2}-\d{2}$/ ) {
        return [ 'Skipping record, could not parse a valid start date.' ];
    }
    if ( not_empty($record->{ft_end_date}) && $record->{ft_end_date} !~ /^\d{4}-\d{2}-\d{2}$/ ) {
        return [ 'Skipping record, could not parse a valid end date.' ];
    }
