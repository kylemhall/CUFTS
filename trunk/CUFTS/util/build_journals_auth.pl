...
sub load_journals_dual_issn {
	...
	my $journals = CUFTS::DB::Journals->search_where('issn' => {'!=', undef}, 'e_issn' => {'!=' => undef}, 'journal_auth' => undef);
...
sub load_journals_e_issn {
	...
	my $journals = CUFTS::DB::Journals->search_where('e_issn' => {'!=', undef}, 'issn' => undef, 'journal_auth' => undef);
...
sub load_local_journals {
	...
	my $journals = CUFTS::DB::LocalJournals->search_where('issn' => {'!=', undef}, 'e_issn' => {'!=' => undef}, 'journal_auth' => undef, 'journal' => undef);
	...
	$journals = CUFTS::DB::LocalJournals->search_where('issn' => {'!=', undef}, 'e_issn' => undef, 'journal_auth' => undef, 'journal' => undef);
	...
	$journals = CUFTS::DB::LocalJournals->search_where('e_issn' => {'!=', undef}, 'issn' => undef, 'journal_auth' => undef, 'journal' => undef);
...
sub process_journal {
	...
	$journal->e_issn and
		!grep $journal->e_issn, @issn_search and
	...
	if (scalar(@issn_search)) {
		...
		@journal_auths = CUFTS::DB::JournalsAuth->search_where(title => {'ilike' => $journal->title});
	}
	...
	if (scalar(@journal_auths) > 1) {
		...
		warn("Multiple journal_auth matches for ISSN.  This should not happen during the original build");
		...
		my $title_rec = CUFTS::DB::JournalsAuthTitles->find_or_create({'title' => $title, 'journal_auth' => $journal_auth->id});
		...
		foreach my $issn (@issn_search) {
			if (!grep {$issn eq $_} @journal_auth_issns) {
				$journal_auth->add_to_issns({issn => $issn, info => 'CUFTS (initial load)'});
			}
		}
		...
		my $journal_auth = CUFTS::DB::JournalsAuth->create({'title' => $title});
		
		CUFTS::DB::JournalsAuthTitles->create({'journal_auth' => $journal_auth->id, 'title' => $title, 'title_count' => 1});
...
sub create_MARC_records {
	...
	my $timestamp = sprintf("%i4%i2%i2%i2%i2%i2\.0", $year + 1900, $mon + 1, $day, $hour, $min, $sec);
	...
	while (my $auth = $auths->next) {
		...
		foreach my $issn (map {$_->issn} $auth->issns) {
			substr($issn, 4, 0) = '-';
			my $issn_field = MARC::Field->new('022', '#', '#', 'a' => $issn);
		...
		my $title_field = MARC::Field->new(245, 0, $lead_chars, 'a' => $title);
