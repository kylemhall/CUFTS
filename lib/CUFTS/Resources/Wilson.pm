...
##
## Copyright Michelle Gauthier - Simon Fraser University (2003-11-04)
##
...
sub clean_data {
	...
	sub get_wilson_date {
		...
		foreach my $date (@dates) {
			next unless $date =~ /(\d{4})\/(\d{2})/;
			my ($year, $month) = ($1, $2);
			$month > 0 or
				$month = 1;
			my $merged_date = "$year$month";
			defined($best_date) or 
				$best_date = $merged_date;
			if ($type eq 'start') {
				$merged_date < $best_date and
					$best_date = $merged_date;
			} elsif ($type eq 'end') {
				$merged_date > $best_date and
					$best_date = $merged_date;
...
sub resource_details_help {
	return {
        	'resource_identifier' => 'This is a code that Wilson uses to uniquely identify '
                  .  "each database. \n\nExample: BAIN is the resource identifier for "
                  .  'Biological and Agricultural Index Plus', 

                'auth_name' => 'If there is a need to disambiguate overlapping TCP/IP ranges for sites '
...
sub build_linkFulltext {
	...
	foreach my $record (@$records) {
		...
		my $url = 'http://vnweb.hwwilsonweb.com/hww/jumpstart.jhtml?';
                ...
                $url .= ('CustID=' . $resource->auth_name . '&') if $resource->auth_name;               

                $url .= 'genre=article&sid=HWW:' . $resource->resource_identifier;
		$url .= ($record->issn) ? '&issn=' . substr($record->issn, 0, 4) . '-' . substr($record->issn, 4,4)
					: '&title=' . uri_escape($record->title);
                if($request->spage) {
		        $url .= '&volume=' . $request->volume . '&issue=' . $request->issue;
