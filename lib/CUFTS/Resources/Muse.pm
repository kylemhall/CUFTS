...
sub clean_data {
	...
	defined($record->{'issn'}) and
		$record->{'issn'} =~ s/[^0-9xX]//g;
	defined($record->{'e_issn'}) and
		$record->{'e_issn'} =~ s/[^0-9xX]//g;

	if (defined($record->{'___start'}) && $record->{'___start'} =~ /vol\.\s*(\d+)/i) {
		$record->{'vol_ft_start'} = $1;
	}
	if (defined($record->{'___start'}) && $record->{'___start'} =~ /no\.\s*(\d+)/i) {
		$record->{'iss_ft_start'} = $1;
	}
	if (defined($record->{'___start'}) && $record->{'___start'} =~ /issue\s*(\d+)/i) {
		$record->{'iss_ft_start'} = $1;
	}

	if (defined($record->{'___end'}) && $record->{'___end'} =~ /vol\.\s*(\d+)/i) {
		$record->{'vol_ft_end'} = $1;
	}
	if (defined($record->{'___end'}) && $record->{'___end'} =~ /no\.\s*(\d+)/i) {
		$record->{'iss_ft_end'} = $1;
	}
	if (defined($record->{'___end'}) && $record->{'___end'} =~ /issue\s*(\d+)/i) {
		$record->{'iss_ft_end'} = $1;
	}

	if (defined($record->{'___start'}) && $record->{'___start'} =~ /\((.+)\s+(\d{4}).*\)/) {
		my $month = $1;
	...
	} elsif (defined($record->{'___start'}) && $record->{'___start'} =~ /,\s+(\d{4}).*/) {
		$record->{'ft_start_date'} = $1;
	} elsif (defined($record->{'___start'}) && $record->{'___start'} =~ /\((\d{4}).*\)/) {
		$record->{'ft_start_date'} = $1;
	}

	if (defined($record->{'___end'}) && $record->{'___end'} =~ /\((.+)\s+.*(\d{4})\)/) {
		my $month = $1;
	...
	} elsif (defined($record->{'___end'}) && $record->{'___end'} =~ /\(.*(\d{4})\)/) {
		$record->{'ft_end_date'} = $1;
	} elsif (defined($record->{'___end'}) && $record->{'___end'} =~ /,\s+.*(\d{4})/) {
		$record->{'ft_end_date'} = $1;
	}
...
sub can_getTOC {
	...
	return 1 if $class->SUPER::can_getTOC($request) && 
...
sub build_linkTOC {
	...
	foreach my $record (@$records) {
		...
		next unless assert_ne($request->volume);
		next unless assert_ne($request->issue);

		my $url = $record->journal_url;
		$url .= '/toc/' . $record->db_identifier . $request->volume . '.' . $request->issue . '.html';
