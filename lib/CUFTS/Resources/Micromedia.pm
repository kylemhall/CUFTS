...
sub build_linkFulltext {
        ...
        foreach my $record (@$records) {
                ...
                if(assert_ne($request->spage) && assert_ne($request->volume) && 
                     assert_ne($record->issn) && assert_ne($request->issue)) {
          
                   $temp_url = $base_url;
                }
                elsif(assert_ne($record->title) && assert_ne($request->atitle)) { 
                   $temp_url = 'http://gateway.proquest.com/openurl?ctx_ver=Z39.88-2003';
                   $temp_url .= '&res_id=xri:pqd&rft_val_fmt=ori:fmt:kev:mtx:journal&genre=article&';
...
sub build_linkTOC {
        ...
        foreach my $record (@$records) {
                ...
                if(assert_ne($record->issn) && assert_ne($request->volume) &&  assert_ne($request->issue)) {          
                   $temp_url = $base_url;
                }
                elsif(assert_ne($record->title) && assert_ne($request->date)) { 
                   $temp_url = 'http://gateway.proquest.com/openurl?ctx_ver=Z39.88-2003';
                   $temp_url .= '&res_id=xri:pqd&rft_val_fmt=ori:fmt:kev:mtx:article&';
                   push (@params, 'jtitle=' . $record->title);
