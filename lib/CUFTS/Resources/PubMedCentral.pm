...
sub clean_data {
	...
	if (defined($record->{'___Earliest volume'})) {
		...
		defined($date) or
			$date = $record->{'___Earliest volume'};

		$date =~ /(\d{4})/ and
			$record->{'ft_start_date'} = $1;

		defined($vol) && $vol =~ /v\.\s*(\d+)/ and
	...
	if (defined($record->{'___Latest issue'})) {
		...
		if ($date =~ /(\d{4})/ && $1 ne '2004') {
			...
			defined($vol) && $vol =~ /v\.\s*(\d+)/ and
				$record->{'vol_ft_end'} = $1;
			defined($vol) && $vol =~ /\((\d+)/ and
				$record->{'iss_ft_end'} = $1;
		}
	}

	defined($record->{'embargo_months'}) && $record->{'embargo_months'} =~ /no\s*delay/i and
		delete $record->{'embargo_months'};

	defined($record->{'embargo_months'}) && $record->{'embargo_months'} =~ /(\d+)\s*months/i and
