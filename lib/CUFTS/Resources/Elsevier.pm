...
sub clean_data {
	...
	if (defined($record->{'___Coverage Begins'})) {
		if ($record->{'___Coverage Begins'} =~ /vol[^0-9]+([0-9]+)/i) {
			$record->{'vol_ft_start'} = $1;
		}
		if ($record->{'___Coverage Begins'} =~ /iss[^0-9]+([0-9]+)/i) {
			$record->{'iss_ft_start'} = $1;
		}
		
		if ($record->{'___Coverage Begins'} =~ /\(\s*(\d+)\s+(\w+)\s+(\d+)\s*\)/i) {
			my ($day, $month, $year) = ($1, $2, $3);
			$month = get_month($month, 'start');
			$record->{'ft_start_date'} = sprintf("%04d-%02d-%02d", $year, $month, $day);
		} elsif ($record->{'___Coverage Begins'} =~ /\(\s*([a-zA-Z]+)\s*-?\s*[a-zA-Z]*\s+(\d+)\s*\)/i) {
			my ($month, $year) = ($1, $2);
			$month = get_month($month, 'start');
			$record->{'ft_start_date'} = sprintf("%04d-%02d", $year, $month);
		} elsif ($record->{'___Coverage Begins'} =~ /\(\s*(\d+)/i) {
			$record->{'ft_start_date'} = $1;
		}
	}

	if (defined($record->{'___Coverage Ends'})) {
		if ($record->{'___Coverage Ends'} =~ /vol[^0-9]+[0-9]+-([0-9]+)/i) {
			$record->{'vol_ft_end'} = $1;
		} elsif ($record->{'___Coverage Ends'} =~ /vol[^0-9]+([0-9]+)/i) {
			$record->{'vol_ft_end'} = $1;
		}
		if ($record->{'___Coverage Ends'} =~ /iss[^0-9]+[0-9]+-([0-9]+)/i) {
			$record->{'iss_ft_end'} = $1;
		} elsif ($record->{'___Coverage Ends'} =~ /iss[^0-9]+([0-9]+)/i) {
			$record->{'iss_ft_end'} = $1;
		}
		
		if ($record->{'___Coverage Ends'} =~ /\(\s*(\d+)\s+(\w+)\s+(\d+)\s*\)/i) {
			my ($day, $month, $year) = ($1, $2, $3);
			$month = get_month($month, 'end');
			$record->{'ft_end_date'} = sprintf("%04d-%02d-%02d", $year, $month, $day);
		} elsif ($record->{'___Coverage Ends'} =~ /\(\s*[a-zA-Z]*?\s*-?\s*([a-zA-Z]+)\s+(\d+)\s*\)/i) {
			my ($month, $year) = ($1, $2);
			$month = get_month($month, 'end');
			$record->{'ft_end_date'} = sprintf("%04d-%02d", $year, $month);
		} elsif ($record->{'___Coverage Ends'} =~ /(\d+)\s*\)/i) {
