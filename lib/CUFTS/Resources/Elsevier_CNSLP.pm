...
sub clean_data {
	...
	if ($record->{'___Date Coverage'} =~ /^(.+(?:\)))\s*-\s*(.+)$/) {
		($start, $end) = ($1, $2);
	} elsif ($record->{'___Date Coverage'} =~ /^(.+(?:\d))\s+-\s*(.+)$/) {
		($start, $end) = ($1, $2);
	} elsif ($record->{'___Date Coverage'} =~ /^(.+(?:\d))\s*-\s*((?:v).+)$/) {
		($start, $end) = ($1, $2);
	} else {
