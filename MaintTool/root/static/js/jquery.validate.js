	...
	validate: function( options ) {
		...
		if ( validator.settings.event ) {
			...
			validator.elements.bind( validator.settings.event, function() {
				validator.element( this, validator.settings.event );
			} );
		}
...
jQuery.extend(jQuery.validator, {
	...
	prototype: {
		 ...
		 * @cat Plugins/Validate
		 */
		element: function( element, event_string ) {
			this.prepareElement( element );
			this.check( element, event_string );
			this.showErrors();
		},
		...
		check: function( element, event_string ) {
			element = this.clean( element );
			jQuery( element ).removeClass( this.settings.errorClass );
			var rules = this.rules( element );
			for( var i = 0, rule; rule = rules[i++]; ) {
				try {
					var result = jQuery.validator.methods[rule.method]( jQuery.trim(element.value), element, rule.parameters, event_string );
					if( result === -1 )
						break;
					if( !result ) {
	...
	methods: {
		 ...
		 * @cat Plugins/Validate/Methods
		 */
		required: function(value, element, param, event_string) {
			// check if dependency is met
			if ( !jQuery.validator.depend(param, element) )
				return -1;
			if ( event_string )   /* Do not check required for non-submit events */
			    return 1;
			switch( element.nodeName.toLowerCase() ) {
			case 'select':
